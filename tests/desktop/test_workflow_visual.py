"""Visual regression baseline capture for WorkflowPage.

Captures ``tests/desktop/baselines/workflow.png`` so future UI polish
work can detect regressions in the workflow surface.  Runs under
``QT_QPA_PLATFORM=offscreen`` (set in ``pyproject.toml``).

Usage:
    pytest tests/desktop/test_workflow_visual.py -v
"""
...
def visual_workspace(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Iterator[Path]:
    """Deterministic workspace + QSS setup for the workflow snapshot."""
...
def _render_workflow(page, qtbot: QtBot, storage_root: Path) -> None:
    """Bind a deterministic workspace snapshot to the workflow page."""
...
class TestWorkflowVisualBaseline:
    """WorkflowPage visual regression baseline (Task 18)."""

    def test_workflow_baseline_captured(
        self,
        qtbot: QtBot,
        ...
        _render_workflow(page, qtbot, visual_workspace)

        image = capture_widget_screenshot(page)
        baseline_path = save_baseline("workflow", image)
