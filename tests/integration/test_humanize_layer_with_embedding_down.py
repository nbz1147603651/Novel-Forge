"""Integration: humanize layer with embedder failure.

When the embedder raises during retrieval, the retriever catches it
internally and returns an empty list.  The layer continues with empty
library_hits — no crash, scan still runs.
"""
...
async def test_embedder_down_degrades_gracefully(
    mock_runner: MagicMock,
    mock_bundle: MagicMock,
) -> None:
    """Embedder raises → retriever returns empty, scan still runs."""
    ...
    mock_retriever_instance.retrieve.side_effect = RuntimeError("embedder down")
