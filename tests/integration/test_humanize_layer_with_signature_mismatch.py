"""Integration: humanize layer with embedding signature mismatch.

When the library's embedding signature doesn't match the configured
embedder, the retriever degrades to FTS5-only mode.  The layer
continues normally — no crash, scan still runs.
"""
...
async def test_signature_mismatch_degrades_to_fts5(
    mock_runner: MagicMock,
    mock_bundle: MagicMock,
) -> None:
    """Embedding signature mismatch → retriever degrades, scan still runs."""
    ...
    mock_library = MagicMock(spec=HumanizeLibrary)
    mock_library.is_healthy.return_value = True
    ...
    mock_retriever_instance.retrieve.return_value = fts5_only_hits
    ...
    with (
        ...
        patch(
            "novel_forge.memory.humanize_library_store.seed_builtin_patterns",
        ),
        patch(
    ...
    assert len(scan_input.library_hits) == 1
    assert scan_input.library_hits[0]["pattern_id"] == "ai_vocabulary"
