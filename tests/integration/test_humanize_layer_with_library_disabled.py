"""Integration: humanize layer with library disabled.

When ``humanize_library_enabled=False``, the library retrieval path
must be skipped entirely — no ``HumanizeLibrary.from_default_path()``
call, no retrieval, no bump.  The 21 hard-rule scan still runs.
"""
...
def mock_runner():
    ...
    s.humanize_library_enabled = False
...
async def test_library_disabled_skips_retrieval(
    mock_runner: MagicMock,
    mock_bundle: MagicMock,
) -> None:
    """humanize_library_enabled=False → no library calls, scan runs with empty hits."""
    ...
    assert scan_input.library_enabled is False
