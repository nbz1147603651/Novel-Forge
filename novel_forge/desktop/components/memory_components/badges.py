...
Auto-generated in the M3.8 split. Contains badges.py classes.
"""
...
class MemoryBadge(Badge):
    """Tone-aware capsule badge for memory status indicators.

    Task 23 polish: status transitions fade in over 200ms via
    ``windowOpacity`` (D1-safe on macOS, composes with QSS border —
    same pattern as Toast.fade_in Task 12 / MessageBoxDialog Task 13).
    """

    STATUS_FADE_DURATION_MS: int = 200
    tone_map = {
        "default": "default",
        "active": "primary",
        "warning": "warning",
    ...
    def __init__(
        ...
        text: str = "",
        *,
        memory_status: str = "default",
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(text, tone="default", parent=parent)
        # _current_status must be seeded to the requested status so the
        # initial tone assignment is treated as a baseline, not a transition.
        self._current_status: str = memory_status
        self._status_fade_anim: QPropertyAnimation | None = None
        tone = self.tone_map.get(memory_status, "default")
        self.set_tone(tone)

    def set_memory_status(self, status: str) -> None:
        tone = self.tone_map.get(status, "default")
        self.set_tone(tone)
        if status != self._current_status:
            self._current_status = status
            self._animate_status_fade()

    def _animate_status_fade(self) -> None:
        from novel_forge.desktop.constants import ANIMATIONS_ENABLED

        if not ANIMATIONS_ENABLED:
            return

        self._stop_status_fade()

        anim = QPropertyAnimation(self, b"windowOpacity", self)
        anim.setDuration(self.STATUS_FADE_DURATION_MS)
        anim.setStartValue(0.6)
        anim.setEndValue(1.0)
        anim.setEasingCurve(QEasingCurve.Type.OutCubic)
        anim.finished.connect(lambda: self._clear_status_fade_anim(anim))
        self._status_fade_anim = anim
        anim.start(QAbstractAnimation.DeletionPolicy.KeepWhenStopped)

    def _stop_status_fade(self) -> None:
        anim = self._status_fade_anim
    ...
    def _clear_status_fade_anim(self, anim: QPropertyAnimation) -> None:
        if self._status_fade_anim is anim:
            self._status_fade_anim = None
        try:
            anim.deleteLater()
        except RuntimeError:
