...
Auto-generated in the M3.8 split. Contains motifs.py classes.
"""
...
class MemoryMotifCard(Surface):
    """Card displaying a single motif with occurrence count.
    ...
    def __init__(
        ...
        *,
        ...
        self._build_ui(category, description, occurrence_count, last_chapter, status, name)
    ...
    def _build_ui(
        ...
        category_label.setObjectName("memoryCategory")
        header.addWidget(category_label)

        if status == "active":
            status_badge = Badge("活跃", tone="success")
            status_badge.setObjectName("memoryIssueBadge")
            header.addWidget(status_badge)
        elif status == "recall":
            status_badge = Badge("待回调", tone="warning")
            status_badge.setObjectName("memoryIssueBadge")
            header.addWidget(status_badge)

        header.addStretch()

        count_label = QLabel(f"×{occurrence_count}")
        count_label.setObjectName("memoryOccurrence")
        ...
        if name:
            ...
            name_label.setObjectName("memoryMotifName")
            layout.addWidget(name_label)

        if description:
            desc_label = QLabel(description)
            desc_label.setObjectName("memoryHint")
            desc_label.setWordWrap(True)
            layout.addWidget(desc_label)

        if last_chapter > 0:
            chapter_label = QLabel(f"上次：第 {last_chapter} 章")
            chapter_label.setObjectName("memoryLastChapter")
