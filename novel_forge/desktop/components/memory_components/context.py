...
Auto-generated in the M3.8 split. Contains context.py classes.
"""
...
class ContextOverviewCard(Surface):
    """Compact card for context overview (checkpoint, carry-forward, etc.)."""
    ...
    def _build_ui(self, content: str) -> None:
        ...
        title_label.setObjectName("memoryContextTitle")
        if self._highlight:
            title_label.setProperty("highlight", "true")
        layout.addWidget(title_label)

        self._content_label = QLabel(content)
        self._content_label.setObjectName("memoryBody")
        self._content_label.setWordWrap(True)
        layout.addWidget(self._content_label, 1)

    def set_content(self, content: str) -> None:
        self._content_label.setText(content if content else "—")
