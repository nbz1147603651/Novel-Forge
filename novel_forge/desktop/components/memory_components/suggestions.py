...
Auto-generated in the M3.8 split. Contains suggestions.py classes.
"""
...
class MemorySuggestionCard(Surface):
    """Compact card displaying a motif suggestion with priority indicator.

    The card opts into a QSS-only hover state. It deliberately avoids
    graphics effects and scale painting because those can drop child labels
    inside macOS/Qt scroll-area compositing.
    """
    ...
    def __init__(
        ...
        *,
        ...
        _stabilize_memory_surface(self, hover=True)
        self._motif_id = motif_id
        self._priority = priority
        self._build_ui(suggestion, category, motif_name)

    def _build_ui(self, suggestion: str, category: str, motif_name: str = "") -> None:
        layout = QHBoxLayout(self)
        layout.setContentsMargins(10, 8, 10, 8)
        layout.setSpacing(8)

        priority_dot = QFrame()
        priority_dot.setObjectName("memoryPriorityDot")
        priority_dot.setProperty("priority", self._priority)
        priority_dot.setFixedSize(6, 6)
        ...
        if category:
            ...
            cat_label.setObjectName("memoryPriorityCat")
            cat_label.setProperty("priority", self._priority)
            content.addWidget(cat_label)

        if motif_name:
            name_label = QLabel(motif_name)
            name_label.setObjectName("memoryPriorityName")
            name_label.setProperty("priority", self._priority)
            name_label.setWordWrap(True)
            content.addWidget(name_label)

        suggestion_label = QLabel(suggestion)
        suggestion_label.setObjectName("memoryHint")
        suggestion_label.setWordWrap(True)
        content.addWidget(suggestion_label)
