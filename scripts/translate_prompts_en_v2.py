#!/usr/bin/env python3
"""Comprehensive ZH→EN translation for Jinja2 prompt templates.

Re-translates from zh source with extensive dictionary + pattern rules.
"""
...
# ── Ordered translation pairs (longest match first to avoid partial replaces) ──
# These are applied in order; each pair is (zh_text, en_text).
# We use exact substring matching to preserve Jinja2 syntax.
...
PHRASE_MAP: list[tuple[str, str]] = [
    ...
    # === COMMON LABELS / FIELD DESCRIPTIONS ===
...
# Regex patterns to identify lines that should NOT be translated
JINJA_VAR_RE = re.compile(r'\{\{.*\}\}')
JINJA_TAG_RE = re.compile(r'\{%.*%\}')
JINJA_COMMENT_RE = re.compile(r'\{#.*?#\}', re.DOTALL)
FIELD_NAME_RE = re.compile(r'^[\w_]+\s*[:=]')
